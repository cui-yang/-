...
sed32 -i "s/^\"//; s/\"$//" tmp.txt
if exist "%~dp0tool\json指定行关键词.txt" (goto HB) else (goto TG)

:HB
(for /f "usebackq delims=" %%k in ("%~dp0tool\json指定行关键词.txt") do (
...
echo ------------------------------------------------------------
echo json输出准备...
echo y|del /s "%~dp0json成果">nul 2>nul
xcopy /q /s "%~dp0json\*" "%~dp0json成果\">nul 2>nul
echo ------------------------------------------------------------
