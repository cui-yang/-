...
set /p "xmm="
:JC1
(dir /a /b "%~dp0txt_json\*" | findstr /R "." >nul && (goto JC2) || (goto JC9))>nul 2>nul
:JC2
if exist "%~dp0项目文件\%xmm%_txt_json_样例.txt" (goto JC7) else (goto JC6)
:JC3
(dir /a /b "%~dp0项目文件\%xmm%_txt_json备份\*" | findstr /R "." >nul && (goto JC5) || (goto TC))>nul 2>nul
:JC4
echo ------------------------------------------------------------
...
if exist "%~dp0%xmm%_%%k_txt文本.txt" (goto JC8) else (goto JC6))
:JC8
echo ------------------------------------------------------------
echo 该项目已存在，请为项目换个名称！！！
echo 若直接按回车键，将会覆盖旧项目！！！
...
("%~dp0tool\ssed.exe" -r -e "s/^\s+//g; s/(.*)\x20\x3d\x20\x22.*/\1/" "%~dp0项目文件\%xmm%_txt_json_样例.txt" | "%~dp0tool\mawk.exe" "!x[$0]++" > "%~dp0项目文件\tmp0.txt")>nul 2>nul
for /f "usebackq delims=" %%k in ("%~dp0项目文件\tmp0.txt") do (
if exist "%~dp0%xmm%_%%k_txt文本.txt" (goto JC4) else (goto JC3))
:SZ
echo ------------------------------------------------------------
