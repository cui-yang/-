...
set /p "xmm="
:JC1
(dir /a /b "%~dp0xml\*" | findstr /R "." >nul && (goto JC2) || (goto JC5))>nul 2>nul
:JC2
if exist "%~dp0项目文件\%xmm%_xml样例.txt" (goto JC3) else (goto SZ1)
:JC3
...
if exist "%~dp0%xmm%_%%k_xml文本.txt" (goto JC4) else (goto SZ1))
:JC4
echo ------------------------------------------------------------
...
goto TQ
:JC5
(dir /a /b "%~dp0项目文件\%xmm%_xml备份\*" | findstr /R "." >nul && (goto JC6) || (goto TC))>nul 2>nul
:JC6
echo ------------------------------------------------------------
echo xml文件夹中没有文件？？？
...
:JC7
if exist "%~dp0项目文件\%xmm%_xml样例.txt" (goto JC8) else (goto SZ2)
:JC8
("%~dp0tool\ssed.exe" -r "s/.*\x3c(.*)\x3e.*\x3c\x2f.*\x3e.*/\1/" "%~dp0项目文件\%xmm%_xml样例.txt" | "%~dp0tool\mawk.exe" "!x[$0]++" > "%~dp0项目文件\tmp0.txt")>nul 2>nul
for /f "usebackq delims=" %%k in ("%~dp0项目文件\tmp0.txt") do (
if exist "%~dp0%xmm%_%%k_xml文本.txt" (goto JC8) else (goto SZ2))
:JC9
echo ------------------------------------------------------------
echo 该项目已存在，请为项目换个名称！！！
echo 若直接按回车键，将会覆盖旧项目！！！
